...
const DEPLOY_COLLECTION = gql`
  mutation DeployCollection($deployCollectionId: ID!) {
    deployCollection(id: $deployCollectionId)
  }
`;

const START_MINTING = gql`
  mutation StartMinting($startMintingId: ID!) {
    startMinting(id: $startMintingId)
  }
`;

const CollectionInfo = ({ collection, refetch }) => {
  const [deploying, setDeploying] = useState(false);
  const bgGradient = { background: gradient(collection._id) };

  const [deployCollection] = useMutation(DEPLOY_COLLECTION);
  const [startMinting] = useMutation(START_MINTING);
        ...
        >
          ...
          >
            ...
            <IconButton
              ...
              `}
              size="small"
              disabled={collection.contractAddress || deploying}
              onClick={() => {
                setDeploying(true);
                deployCollection({
                  variables: { deployCollectionId: collection._id },
                })
                  .then((r) => {
                    refetch();
                    setDeploying(false);
            ...
            <IconButton
              ...
              onClick={() => {
                console.log("here??");
                startMinting({ variables: { startMintingId: collection._id } })
                  .then((r) => {
                    console.log(r);
                  })
            ...
            >
              {collection.contractAddress ? (
                <a
                  ...
                  target="_black"
                  href={`https://devnet.venomscan.com/accounts/${collection.contractAddress}`}
                >
                  {collection.contractAddress}
                </a>
              ) : (
